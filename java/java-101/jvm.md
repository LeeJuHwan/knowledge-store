...
CPU 는 컴퓨터의 핵심 부품으로 머신 코드를 이해한다. 하지만, 머신 코드로 변환 하는 과정은 플랫폼에 의존적이다.

자바는 어떤 플랫폼이든 실행할 수 있는 환경을 제공하는 것이 철학이기 때문에 <mark style="color:red;">**플랫폼에 종속적이지 않도록 구성하기 위해 CPU 와 같은 하드웨어를 소프트웨어로 모방하여 연산과 메모리 관리를 할 수 있도록 만들었다.**</mark>

<mark style="color:red;">**이 때, 이 소프트웨어로 CPU 를 모방한 것이 JVM 이며 JVM 만의 머신 코드가 바로 자바 바이트코드이다.**</mark>

그렇기 때문에 자바 소스 코드를 자바 바이트코드로 컴파일하면, JVM이 인터프리터와 JIT 컴파일러를 통해 각 플랫폼의 머신 코드로 변환하여 CPU가 직접 연산하도록 한다.

이때 JVM 자체는 각 플랫폼에 맞는 JVM을 설치해야 하지만, 자바 바이트코드는 어떤 플랫폼의 JVM에서도 동일하게 실행될 수 있어 플랫폼에 종속적이지 않게 되었다.
{% endhint %}
...
JVM 은 하드웨어를 논리적으로 모방한 소프트웨어라면, 결국 이 모방된 하드웨어도 전원을 눌러 부팅이 필요하다. JVM 이 동작하기 위해 전원을 누른 것과 같은 것이 클래스 로더이며, 내부적으로는 부트스트랩 클래스로더가 이에 해당한다.

즉, 클래스 로더는 클래스를 메모리에 로드하고 실행을 위해 사용할 수 있게 만드는 JVM의 일부다.
